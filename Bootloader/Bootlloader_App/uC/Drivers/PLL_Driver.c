...
void PLL_Init(void)
{
	...
	RCC->CFGR2 &=~(RCC_CFGR2_HPRE_Msk | RCC_CFGR2_PPRE1_Msk | RCC_CFGR2_PPRE2_Msk | RCC_CFGR2_PPRE3_Msk);

	RCC->PLL1CFGR &=~(RCC_PLL1CFGR_PLL1VCOSEL | RCC_PLL1CFGR_PLL1SRC | RCC_PLL1CFGR_PLL1RGE);
	
	/* Clear the PLL Multiplication factor*/
	RCC->PLL1CFGR |= RCC_PLL1CFGR_PLL1M_1 ;
		
	/*  set the PLL clock (HSI)*/
	RCC->PLL1CFGR |= (RCC_PLL1CFGR_PLL1SRC_0  //HSI selected
	...
	RCC->PLL1DIVR	&=~(RCC_PLL1DIVR_PLL1P | RCC_PLL1DIVR_PLL1R | RCC_PLL1DIVR_PLL1Q  | RCC_PLL1DIVR_PLL1N);
